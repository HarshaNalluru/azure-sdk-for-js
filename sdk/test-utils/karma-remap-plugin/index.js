...
function KarmaRemapPlugin(baseReporterDecorator, logger, config) {
  baseReporterDecorator(this);
  console.log(this);
  const log = logger.create("reporter.remap-plugin");

  const remapPluginReporterConfig = config.remapPluginReporter || {};
  const reports = remapPluginReporterConfig.reports || {};
  const remapOptions = remapPluginReporterConfig.remapOptions || {};
  const reportOptions = remapPluginReporterConfig.reportOptions || {};
...
KarmaRemapPlugin.$inject = ["baseReporterDecorator", "logger", "config"];

module.exports = {
  "reporter:karma-remap-plugin": ["type", KarmaRemapPlugin]
};
