...
describe("MessageIdURL", () => {
  ...
  it.only("update and delete empty message with default parameters", async () => {
    ...
    const eResult = await messagesURL.enqueue(Aborter.none, messageContent);
    ...
    const newMessage = "";
    const messageIdURL = MessageIdURL.fromMessagesURL(messagesURL, eResult.messageId);
    const uResult = await messageIdURL.update(Aborter.none, eResult.popReceipt, 0, newMessage);
    ...
    const dResult = await messageIdURL.delete(Aborter.none, uResult.popReceipt!);
  ...
  it("update and delete message with all parameters", async () => {
    ...
    const messageIdURL = MessageIdURL.fromMessagesURL(messagesURL, eResult.messageId);
    const uResult = await messageIdURL.update(Aborter.none, eResult.popReceipt, 10, newMessage);
    assert.ok(uResult.version);
    assert.ok(uResult.timeNextVisible);
    assert.ok(uResult.date);
    assert.ok(uResult.requestId);
    ...
    const pResult = await messagesURL.peek(Aborter.none);
    assert.equal(pResult.peekedMessageItems.length, 0);

    await delay(11 * 1000); // Sleep 11 seconds, and wait the message to be visible again

    const pResult2 = await messagesURL.peek(Aborter.none);
