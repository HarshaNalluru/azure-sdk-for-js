...
export class ServiceBusClient {
  ...
  createReceiver(
    queueName: string,
    options: { isReceiveAndDeleteMode: true }
  ...
  createReceiver(
    ...
    optionsOrSubscriptionName2?: { isReceiveAndDeleteMode?: true | false } | string,
    options3?: { isReceiveAndDeleteMode?: true | false }
   ...
   *
   * @param {string} queueName The name of the queue to receive from.
   * @param {(CreateSessionReceiverOptions)} [options] Options include receiveMode(defaulted to peekLock), options to create session receiver.
   * @returns {Promise<SessionReceiver<ReceivedMessageWithLock>>}
   * @memberof ServiceBusClient
   */
  ...
  createSessionReceiver(
    queueName: string,
    options: CreateSessionReceiverOptions & { isReceiveAndDeleteMode: true }
  ...
  async createSessionReceiver(
    queueOrTopicName1: string,
    optionsOrSubscriptionName2?:
      | (CreateSessionReceiverOptions & { isReceiveAndDeleteMode?: true | false })
      | string,
    options3?: CreateSessionReceiverOptions & { isReceiveAndDeleteMode?: true | false }
   ...
   *
   * @param {string} queueName The name of the queue to receive from.
   * @param {{ receiveMode?: "peekLock" }} [options] Options to pass the receiveMode, defaulted to peekLock.
   * @returns {Receiver<ReceivedMessageWithLock>}
   * @memberof ServiceBusClient
   */
  ...
  createDeadLetterReceiver(
    ...
    if (receiveMode === "receiveAndDelete") {
      return this.createReceiver(deadLetterEntityPath, { isReceiveAndDeleteMode: true });
    } else {
